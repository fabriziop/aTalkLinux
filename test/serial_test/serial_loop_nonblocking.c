...
This program sends lines of a single random printable character through
...
int main()
{
    ...
    for (int i=0; i < NUMBER_OF_CHARS; i++) {
        ...
        /* generate a random printable char */
	random_data[0] = rand() % 95 + 32;

        /* send random data */
        wrlen = write(fd,random_data,2);
        if (wrlen != 2) {
        ...
        /* receive data */
	do {
            rdlen = read(fd,receive_data,2);
	} while (rdlen < 0);

        if (rdlen > 0) {
            printf("Read %d char: \"%c\"\n",rdlen,receive_data[0]);
        } else if (rdlen < 0) {
	    if (errno != EAGAIN)
