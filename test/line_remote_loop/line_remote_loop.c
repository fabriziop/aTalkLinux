...
int main()
{
	 ...
	 * ATALK_DATA_SIZE_MIN - ATALK_DATA_SIZE_MAX */
	  ...
	  + RANDOM_DATA_SIZE_MIN;
        for (r=0; r < random_data_size; r++)
	    atalk->send_msg_encoded[r] = rand() % 64 + 32;
        
	atalk->send_msg_encoded[r] = '\0';

	printf("tx data: %s\n.",atalk->send_msg_encoded);

        /* try to send random data until success or time out */
	error = atalk_serial_tx(atalk);
	if (error < r)
	    printf("ERROR: send error %d.\n",error);

        /* try to receive data until success or timeout */
